...
function startNewChat(_event=null,prefillMessage=''){state.currentConversationId=null;$('chat-title').textContent='New Conversation';$('messages-container').innerHTML='';state.virtualFiles=[];state.pendingFiles=[];renderFilePreview();if(prefillMessage){$('welcome-screen').classList.add('hidden');$('messages-container').classList.remove('hidden');}else showWelcomeScreen();$('message-input').value=prefillMessage;autoResizeTextarea();$('message-input').focus();renderConversationList();if(window.innerWidth<=980)setSidebar(false);}
